...
const SlideLink: React.FC<{ currentSlide?: { link?: Link; }; }> = ({ currentSlide }) => {
  ...
  if (link && text) {
    return (
      <a className="underline flex-shrink-0" href={url} target={isExternal ? '_blank' : '_self'} rel="noopener noreferrer">
        {text}
      </a>
    );
  }
...
const AnnouncementSlider = () => {
  ...
  return (
    <div className={`relative z-overlay flex items-center announcement-slider transition-all duration-500 transform ${announcements.length ? `translate-x-0` : 'opacity-0 translate-x-2'} ${blurred.primary} ${borderBox} gap-6 text-white font-body`}>
      ...
      <p className="flex md:flex-nowrap flex-wrap gap-x-4 gap-y-1 text-base tracking-wider leading-tight font-light">
